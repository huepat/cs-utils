...
namespace HuePat.Util.Math.Geometry {
    public class Triangle: IFiniteGeometry {
        ...
        public bool ContainsCoplanar(
                Vector3d position) {

            return SameSide(position, Corner1, Corner2, Corner3) 
                && SameSide(position, Corner2, Corner1, Corner3) 
                && SameSide(position, Corner3, Corner1, Corner2);
        }
        ...
        public List<Intersection> Intersect(
            ...
            List<Intersection> result = Plane.Intersect(ray);

            if (result.Count == 0) {
                // includes cases where intersection is line segment
                return result;
            }

            if (ContainsCoplanar(result[0].Position)) {
                return result;
            }
        ...
        public void Rotate(
            ...
            Corner1 = Corner1.RotateCoordinate(rotation, anchorPoint);
            Corner2 = Corner2.RotateCoordinate(rotation, anchorPoint);
            Corner3 = Corner3.RotateCoordinate(rotation, anchorPoint);
        }
        ...
        private bool SameSide(
            ...
            return Vector3d.Dot(
                Vector3d.Cross(d, p1 - A), 
                Vector3d.Cross(d, p2 - A)
            ) >= double.Epsilon;
        }
